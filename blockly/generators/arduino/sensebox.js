...
Blockly.Arduino.sensebox_mqtt_setup = function () {
  ...
  Blockly.Arduino.includes_['library_senseBoxMCU'] = '#include "SenseBoxMCU.h"';
  Blockly.Arduino.includes_['library_adafruitmqtt'] = '#include "Adafruit_MQTT.h"';
  Blockly.Arduino.includes_['library_adafruitmqttclient'] = '#include "Adafruit_MQTT_Client.h"';
...
Blockly.Arduino.sensebox_mqtt_publish = function (block) {
  ...
  Blockly.Arduino.loops_['mqtt_connect'] = 'MQTT_connect();';
...
Blockly.Arduino.sensebox_mqtt_subscribe = function (block) {
  ...
  Blockly.Arduino.userFunctions_['mqtt_' + feed_client + 'callbackFunction'] = `void `+ feed_client + `Callback (double x){
    Serial.println(x);
  }`;
  Blockly.Arduino.setups_['mqtt_' + feed_client + '_callback'] = '' + feed_client + '.setCallback('+feed_client + 'Callback);'; 
  Blockly.Arduino.setups_['mqtt_' + feed_client + '_subscribe'] = 'mqtt.subscribe(&' +feed_client + ');';
  Blockly.Arduino.loops_['mqtt_processPackages'] = 'mqtt.processPackets(10);'; 
  var code = '';
  return code;
};
