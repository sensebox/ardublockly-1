...
/** This WatchDog updates the Toolbox according to the used Blocks */

Ardublockly.toolboxWatchDog = function () {

  var list = Ardublockly.BLOCKRULES;
  var AllBlocks = Ardublockly.workspace.getAllBlocks();
  for (var i = 0; i < AllBlocks.length; i++) {
    for (var j = 0; j < list.blocksInWorkspace.length; j++)
      if (AllBlocks[i].type == list.blocksInWorkspace[j].name) {
        for (var k = 0; k < list.blocksInWorkspace.length; k++) {
          Ardublockly.disableBlockFromToolbox(list.blocksInWorkspace[j].blockToDisable[k]);
        }
      }
      else if (AllBlocks[i].type != list.blocksInWorkspace[j].name) {
        for (var k = 0; k < list.blocksInWorkspace.length; k++) {
          Ardublockly.enableBlockFromToolbox(list.blocksInWorkspace[j].blockToDisable[k]);
        }
      }
  }
  Ardublockly.workspace.updateToolbox(Ardublockly.xmlTree);
...
/** Disable block in Toolbox */
Ardublockly.disableBlockFromToolbox = function (blockToDisable) {
  var xmlBlock = Ardublockly.xmlTree.getElementsByTagName('block');
  for (var i = 0; i < xmlBlock.length; i++) {
    if (xmlBlock[i].getAttribute('type') == blockToDisable) {
      xmlBlock[i].setAttribute('disabled', 'true');
    }
  }
};

/** Enable Block in Toolbox */
Ardublockly.enableBlockFromToolbox = function (blockToEnable) {
  var xmlBlock = Ardublockly.xmlTree.getElementsByTagName('block');
  for (var i = 0; i < xmlBlock.length; i++) {
    if (xmlBlock[i].getAttribute('type') == blockToEnable) {
      xmlBlock[i].setAttribute('disabled', 'false');
    }
  }
};
